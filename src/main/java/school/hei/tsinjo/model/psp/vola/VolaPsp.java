...
public class VolaPsp implements Psp {
  ...
  private Payment toPayment(
    ...
    Instant lastVerificationInstant =
            Optional.ofNullable(volaPayment.getLastPspVerificationInstant()).map(Date::toInstant).orElse(null);

    Instant pspCreation =
            (volaPspPayment == null)
                    ? null
                    : Optional.ofNullable(volaPspPayment.getCreationInstant()).map(Date::toInstant).orElse(null);

    Instant rootCreation = Optional.ofNullable(volaPayment.getCreationInstant()).map(Date::toInstant).orElse(null);
